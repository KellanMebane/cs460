...
int show_bmp(char *p, int startRow, int startCol, int scale)
{
  ...
  for (i = startRow; i * scale < (h + startRow); i += 1)
  {
    pp = p;
    for (j = startCol; j * scale < (startCol + w); j += 1)
    {
      b = *pp;
      g = *(pp + 1);
      r = *(pp + 2);
