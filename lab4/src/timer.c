...
void decrement_timers(TNODE **tqq);
void printTimers(TNODE *tqq);
...
void timer_init()
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		*(tp->base + TMIS) = 0x0;
		*(tp->base + TLOAD) = 0x100;
		*(tp->base + TCNTL) = 0x62; //011-0010=|NOTEn|Pe|IntE|-|scal=00|32-bit|0=wrap|
...
void timer_handler(int n)
{
	...
	if (t->tick >= 256)
	{
		...
		decrement_timers(&activeTimers);
		printTimers(activeTimers);
		//DECREMENT TIMERS
	}
...
void decrement_timers(TNODE **tqq) {
	...
	while (current) {
		...
		if (current->seconds <= 0)
			{
				temp = pop_timer(&activeTimers);
				push_timer(&freeTimers, temp);
				kwakeup(temp->this_proc);
			}
		current = current->tqe;
	}
}
...
void printTimers(TNODE *tqq) {
	while (tqq) {
		printf("[%d %d] -> ", tqq->this_proc->pid, tqq->seconds);
		tqq = tqq->tqe;
		printf("\n");
	}
}